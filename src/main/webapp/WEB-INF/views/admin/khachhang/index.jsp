...
<!-- 	<hr> -->
	<form action="${rootpath }admin/khachhang/search1" method="get">
		<input type="text" name="search1" maxlength="50" placeholder="Tên khách hàng" value="${search1 }" />
	...
	<table border="1">
		...
		<c:forEach items="${lstkh }" var="kh">
			<tr>
				<td align="center">
					<a href="${rootpath }admin/khachhang/detail?ma_kh=${kh.ma_kh }">${kh.ma_kh }</a>
				</td> 
				<td>${kh.ho_ten }</td>
				<td>${kh.tai_khoan }</td>
				<td>${kh.mat_khau }</td>
				...
				<td>${kh.dien_thoai }</td>
				<td>${kh.email }</td>
				<td><fmt:formatDate dateStyle="MEDIUM" value="${kh.ngay_sinh }"/></td>
				<td><fmt:formatDate dateStyle="MEDIUM" value="${kh.ngay_cap_nhat }"/></td>
				<td>${kh.gioi_tinh }</td>
				<td>${kh.tich_diem }</td>
				<td>${(kh.trang_thai == 1)? "Còn hoạt động": "Không hoạt động" }</td>
				<th align="center">
					<a href="${rootpath }admin/khachhang/init-update?ma_kh=${kh.ma_kh }">
						<img src="${rootpath }assets/img/update.png" />
					</a>
				</th>
				<th align="center">
					<a href="${rootpath }admin/khachhang/delete?ma_kh=${kh.ma_kh }">
