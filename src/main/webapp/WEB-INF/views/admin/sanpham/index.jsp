...
<%-- <c:url value="admin/sanpham/" var="rootPath"/> --%>
...
<body>
 ...
 <main role="main">
	...
	<hr>
	<form action="${rootpath }admin/sanpham/search" method="get">
		<input type="text" name="search" maxlength="50"
			placeholder="Tên sản phẩm" value="${s }" /> <input type="submit"
			value="Tìm kiếm" />
	</form>
	<hr>
	<table border="1">
		...
		<c:forEach items="${lstsp }" var="sp" varStatus="loop">
			<tr>
				<td align="center"><a
					href="${rootpath }admin/sanpham/detail?ma_sp=${sp.ma_sp }">${sp.ma_sp }</a>
				</td>
				<td>${sp.ten_sp }</td>
				<td>${sp.thong_tin }</td>
				...
				<td><fmt:formatDate dateStyle="MEDIUM"
						value="${sp.ngay_cap_nhat }" /></td>
				<td>${(sp.trang_thai == 1)? "Còn hàng": "Hết hàng" }</td>
				<td><img class="anh" src="<c:url value ="/assets/img/${sp.img }" />" alt=""
					width="200px" height="200px"></td>
				<td>${sp.loaiSanPham.ten_loai }</td>
				<td>${sp.nhanHieu.ten_nhan_hieu }</td>
				<th align="center"><a
					href="${rootpath }admin/sanpham/init-update?ma_sp=${sp.ma_sp }">
						<img src="${rootpath }assets/img/update.png" />
				</a></th>
				<th align="center"><a
					href="${rootpath }admin/sanpham/delete?ma_sp=${sp.ma_sp }"> <img
						src="${rootpath }assets/img/delete.png" />
				</a></th>
			</tr>
		</c:forEach>
	</table>
	<div class="pagination">
		<c:forEach var="sp" begin="1" end="${paginateInfo1.totalPage }"
			varStatus="loop">
			<c:if test="${(loop.index) == paginateInfo1.currentPage}">
				<a href="${rootpath }admin/sanpham/${ loop.index}" class="active">${loop.index }</a>

			</c:if>
			<c:if test="${(loop.index) != paginateInfo1.currentPage}">
				<a href="${rootpath }admin/sanpham/${ loop.index}">${loop.index }</a>
			</c:if>
		</c:forEach>
	</div>
	<a class="a" href="${rootpath }admin/sanpham/initInsert">Thêm mới</a>	
	<hr>
	</main> 
</body>
</html>